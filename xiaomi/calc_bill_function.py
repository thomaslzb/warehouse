...
def get_fuel_charge_rate(queryset, input_date, ):
    ...
    for record in queryset:
        if datetime.strftime(record.begin_date, '%Y-%m-%d') <= \
                input_date <= datetime.strftime(record.end_date, '%Y-%m-%d'):
...
def ups_delivery_fee(total_delivery, record, item_queryset, is_uk_zone2, fuel_charge_rate):
    ...
    fuel_charge_rate_percent = Decimal(1 + fuel_charge_rate / 100)
    ...
    if fee_desc == 'Dom. Standard':
        ...
        if fee_desc == 'Residential':
            ...
            total_delivery[fee_desc][2] = Decimal(UPS_RESIDENTIAL_FEE)
            total_delivery[fee_desc][3] += round(Decimal(UPS_RESIDENTIAL_FEE) * fuel_charge_rate_percent, 2)
            total_delivery[fee_desc][4] = 90
            item_detail[fee_desc] = Decimal(UPS_RESIDENTIAL_FEE)
        ...
        if fee_desc == 'UK Border Fee':
            ...
            item_detail[fee_desc] = record.ups_fee_amount
        if fee_desc == 'Additional Handling' and record.ups_fee_code == 'ACC':
            total_delivery[fee_desc][0] += 1
            total_delivery[fee_desc][1] += record.total_qty
            total_delivery[fee_desc][2] = record.ups_fee_amount
            total_delivery[fee_desc][3] += amount
...
def dpd_delivery_fee(total_delivery, record, item_queryset, is_uk_zone2, is_dpd_congestion_zone,
    ...
    vat_value = round(Decimal(cost * Decimal(DPD_VAT_RATE)), 2)
...
def calc_handle_bill(self, bill_year, bill_month, handle_recode_queryset, progress_begin, progress_total, ):
    global CALC_PROGRESS_NUMBER
    handle_detail_record = []
    ...
    handle_total_amount = 0  # 处理费汇总
    for_count = 0
    for record in handle_recode_queryset:
        for_count += 1
        CALC_PROGRESS_NUMBER = int(((for_count + progress_begin) / progress_total) * 100)
        ...
        handle_total_amount += total_amount
...
def update_insert_sql(self, insert_sql_list, record, express_company, calc, update_year,
                      update_month, fuel_surcharge_rate, this_bill_no):
    total_amount = 0
    nett_cost = 0
    total_vat = 0
    this_bill_no = this_bill_no[:-1]
    i = 0
    ...
    need_calc_amount = 0   # 需要计算燃油费的项目之和
    for value in calc:
        if i < 9:
            total_amount += value
            if i != 8:  # address_correction_dom_standard 不计算燃油费
                need_calc_amount += value
        elif i == 9:  # cost
            nett_cost = value
        else:  # vat
            total_vat = value
    ...
    fuel_surcharge = round(need_calc_amount * fuel_surcharge_rate / 100, 2)
...
# 根据账单计算快递费用的明细及成本
def calc_express_bill(self, bill_year, bill_month, progress_begin, progress_total, express_company, recode_queryset, ):
    global CALC_PROGRESS_NUMBER
    # s_time = time.time()
    ...
    if bill_month == 1:
        ...
        seek_end_year = bill_year
        seek_end_month = bill_month + 2
    seek_begin_date = datetime.strptime(str(seek_begin_year) + '/' + str(seek_begin_month) + '/01', '%Y/%m/%d')
    seek_end_date = datetime.strptime(str(seek_end_year) + '/' + str(seek_end_month) + '/01', '%Y/%m/%d')
    final_date = datetime.strptime('2030/12/31', '%Y/%m/%d')
    ...
    if recode_queryset:  # 初始化第一个记录
        record = recode_queryset[0]
        to_write_mi_code = record.mi_code
        last_recode = record
    ...
    for record in recode_queryset:
        ...
        if record.bill_no:  # 记录快递公司的账单号，查找到改 mi_code
            ...
            if this_bill_no.find(record.bill_no) == -1:
                this_bill_no += ''.join(record.bill_no) + '-'
        ...
        if to_write_mi_code != record.mi_code and for_count <= total_count:  # 总结上一个 mi_code , 并初始化下一个
            calc = [standard_delivery_fee, additional_fee, residential, dom_standard_undeliverable_return,
                    extended_area_surcharge_destination, uk_border_fee, additional_handling,
                    peak_surcharge_additional_handling, address_correction_dom_standard, delivery_cost, delivery_vat]
            ...
            if express_company == 'UPS':
                ...
                calc_result = update_insert_sql(self, express_detail_record, last_recode,
                                                'UPS', calc, bill_year, bill_month, fuel_charge_rate, this_bill_no)
            else:  # express_company == 'DPD':
                calc_result = update_insert_sql(self, express_detail_record, last_recode,
                                                'DPD', calc, bill_year, bill_month, fuel_charge_rate, this_bill_no)
            # 汇总计算结果
            total_records_num += 1
            if record.bill_year == bill_year and last_recode.bill_month == bill_month:
                this_month_delivery_list.append(last_recode.mi_code)  # 记录当月的记录
            else:
                last_month_delivery_list.append(last_recode.mi_code)  # 记录上月的记录
        ...
        if for_count == total_count and to_write_mi_code == record.mi_code:  # 处理循环的最后一个记录 mi_code
            ...
            if record.bill_year == bill_year and record.bill_month == bill_month:
                this_month_delivery_list.append(record.mi_code)  # 记录当月的记录
            else:
                last_month_delivery_list.append(record.mi_code)  # 记录上月的记录
            express_detail_record = calc_result[0]
    ...
    # e_time = time.time()
    # print(f'{express_company} fee spend times {e_time-s_time}')
...
def calc_dcg_bill(self, bill_year, bill_month):
    ...
    # 获取所有需要计算的的结果集
    # 1 - 获取计算 DPD 的结果集
    qs_mi_dpd = MiAccountBillDetailModel.objects.raw(MI_BILL_DPD_SQL, [bill_year, bill_month])
    count_mi_dpd = len(list(qs_mi_dpd))
    # 2 - 获取计算 UPS 的结果集
    qs_mi_ups = MiAccountBillDetailModel.objects.raw(MI_BILL_UPS_SQL, [bill_year, bill_month])  # 获取结果集
    count_mi_ups = len(list(qs_mi_ups))

    # 3 - 获取计算  DCG-UK Bill - Handle 账单(根据小米的账单，计算出所有的处理费用的明细)
    handle_recode_queryset = MiAccountBillDetailModel.objects.filter(bill_year=bill_year,
                                                                     bill_month=bill_month,
                                                                     ).order_by('bill_year', 'bill_month', 'mi_code')
    count_mi_handle = handle_recode_queryset.count()

    all_total_count = count_mi_dpd + count_mi_ups + count_mi_handle

    # 开始计算
    # # 计算 DCG-UK Bill -  DPD 账单(根据小米的账单，查询ups账单，计算出所有的快递费明细及成本)
    if count_mi_dpd > 0:
        progress_begin = 0
        dpd_bill = calc_express_bill(self, bill_year, bill_month, progress_begin, all_total_count,
                                     express_company='DPD', recode_queryset=qs_mi_dpd, )
    else:
        dpd_bill = [[], [], '', [], [], 0, 0, ]
    ...
    if count_mi_ups > 0:  # 发现有ups的账单
        progress_begin = count_mi_dpd
        ups_bill = calc_express_bill(self, bill_year, bill_month, progress_begin, all_total_count,
                                     express_company='UPS', recode_queryset=qs_mi_ups, )
    ...
    if ups_mi_bill_result_count > 0:
        progress_begin = count_mi_dpd
        all_total_count += ups_mi_bill_result_count
        ups_mi_bill = calc_express_bill(self, bill_year, bill_month, progress_begin, all_total_count,
                                        express_company='UPS', recode_queryset=recode_queryset, )
        ...
        count_mi_ups += ups_mi_bill_result_count
    ...
    progress_begin = count_mi_dpd + count_mi_ups
    handle_Bill = calc_handle_bill(self, bill_year, bill_month, handle_recode_queryset,
                                   progress_begin, all_total_count, )
    handle_detail_record = handle_Bill[1]

    # 更新数据库
    CALC_PROGRESS_NUMBER = 99
...
def calc_progress(request):
    return JsonResponse(CALC_PROGRESS_NUMBER, safe=False)
