...
def get_custom_profit_rate(quote_type, calcVolumeWeight):
    profit = {"fix_profit": 0, "percent_profit": 0}
    if quote_type == "24HOURS":
        item_name = "24Hours"
    else:
        item_name = "48Hours"
    query_set = CustomProfitRate.objects.filter(volume_minimum__lt=calcVolumeWeight,
                                                volume_maximum__gte=calcVolumeWeight,
                                                item_name=item_name)
    ...
    if get_price_list["BasePrice"] != 0 or get_price_list["PlusPrice"] != 0:
        if quote_type == "24HOURS":
            if Profit["fix_profit"] > 0:
                ...
                Delivery_Price = (get_price_list["BasePrice"] + get_price_list["PlusPrice"] * CalcVolumeWeight) * \
                                 Fuel_Surcharge + Profit["fix_profit"]
            else:
                # Calculate as Percent
                Delivery_Price = (get_price_list["BasePrice"] + get_price_list["PlusPrice"] * CalcVolumeWeight) * \
                                 Fuel_Surcharge * (1 + Profit["percent_profit"])
            ...
            if Profit["fix_profit"] > 0:
                ...
                # Calculate as Percent
                Delivery_Price = (get_price_list["BasePrice"] + get_price_list["PlusPrice"]) * CalcVolumeWeight * \
                                 (1 + Profit["percent_profit"])
    else:
        Delivery_Price = 0

    # 查询附加费
    Surcharge = SubchargeCalculate(Delivery_Price, CalcVolumeWeight24)
...
def SubchargeCalculate(delivery_price, calc_volume_weight):
    """ 获取附加费 """
    ...
    for record in query_set:
        surcharge["Airline_Handing_Fee"] = Decimal(record.price * calc_volume_weight)

    if surcharge["Airline_Handing_Fee"] < 50:   # 最少也要收取50.00
        surcharge["Airline_Handing_Fee"] = Decimal(50.00)
    ...
    if surcharge["COVID19_Fee"] < 8:   # 最少也要收取8.00
        surcharge["COVID19_Fee"] = Decimal(8.00)
...
class AirFreightView(View):
    ...
    def post(self, request):
        ...
        if zone_id == 0:
            # 返回错误，邮编不存在
            return render(request, 'air_freight_result.html', {'menu_active': 'AIR',
